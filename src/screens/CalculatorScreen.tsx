...
export function CalculatorScreen() {
  ...
  async function shareDealReport(deal: SavedDeal) {
    ...
    // Native iOS/Android: use the system share sheet
    if (Platform.OS !== 'web') {
      const isBRRR = deal.strategy === 'btl' && deal.inputs.refinanceAfterRefurb === 'yes';
      const message = [
        `📊 ${deal.label} — ${deal.strategy.toUpperCase()}${isBRRR ? ' (BRR)' : ''}`,
        '',
        `Monthly Cash Flow: £${f(deal.monthlyNetCashflow)}`,
        `Cash-on-Cash Return: ${f(deal.cashOnCash, 1)}%`,
        `Total Invested: £${f(deal.totalInvested)}`,
        isBRRR
          ? `Capital Left In: £${f(deal.capitalLeftIn ?? 0)}`
          : `Gross Yield: ${f(deal.grossYield, 1)}%`,
        '',
        `Purchase Price: £${f(Number(deal.inputs.purchasePrice) || 0)}`,
        `Monthly Rent: £${f(Number(deal.inputs.rentPerMonth) || 0)}`,
        `Monthly Mortgage: £${f(deal.monthlyMortgage)}`,
        `Net Yield: ${f(deal.netYield, 1)}%`,
        `Stamp Duty: £${f(deal.stampDuty)}`,
        '',
        'Generated by Property Deal Calculator',
      ].join('\n');
      try {
